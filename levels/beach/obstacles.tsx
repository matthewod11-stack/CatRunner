...
import type { EntityType, Obstacle, ObstacleType } from '../../types';
...
const BEACH_OBSTACLE_TYPES = new Set<ObstacleType>([
  'CRAB',
  'BEACHBALL',
  'SEAGULL',
  'SANDCASTLE',
...
export function isBeachObstacleType(type: EntityType): boolean {
  return BEACH_OBSTACLE_TYPES.has(type as ObstacleType);
}

const processedSpriteCache = new Map<string, string>();
const spriteProcessingCache = new Map<string, Promise<string>>();

const MAX_PROCESSED_SPRITE_CACHE = 12;

function rememberProcessedSprite(src: string, result: string): void {
  if (processedSpriteCache.has(src)) processedSpriteCache.delete(src);
  processedSpriteCache.set(src, result);
  while (processedSpriteCache.size > MAX_PROCESSED_SPRITE_CACHE) {
    const first = processedSpriteCache.keys().next().value;
    if (first === undefined) break;
    processedSpriteCache.delete(first);
  }
}
...
function processSprite(src: string): Promise<string> {
  ...
  const processingPromise = new Promise<string>((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      rememberProcessedSprite(src, result);
...
function renderBeachObstacleIcon(obstacle: Obstacle): React.ReactNode {
    ...
    case 'SANDCASTLE':
      return (
        <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-lg">
          ...
          <path
            d="M8 95 Q 12 78 22 73 Q 32 68 42 70 Q 52 72 58 70 Q 68 67 78 73 Q 88 78 92 95 Z"
            fill="url(#sandLight)"
            opacity="0.6"
          />
          ...
          <g transform="translate(5, 75)">
            ...
            <path
              d="M-3 18 Q -3 12 2 12 Q 7 12 7 18 Q 7 22 2 22 Q -3 22 -3 18 Z"
              fill="#3b82f6"
              stroke="#1e40af"
              strokeWidth="1"
            />
      ...
      return (
        <div className="w-full h-full relative" style={{ transform: `rotate(${rotation}deg)` }}>
          <div className="absolute inset-0 flex items-center justify-center">
            <div
              className="absolute w-2 h-2 bg-amber-400/60 rounded-full blur-sm anim-trail-pulse"
              style={{ left: '10%', top: '20%' }}
            />
            <div
              className="absolute w-1.5 h-1.5 bg-amber-500/60 rounded-full blur-sm anim-trail-pulse"
              style={{ left: '15%', top: '30%', animationDelay: '0.1s' }}
            />
          </div>
...
export const BeachObstacleIcon = memo(
  function BeachObstacleIcon({ obstacle }: { obstacle: Obstacle }) {
    return <>{renderBeachObstacleIcon(obstacle)}</>;
  },
  (prev, next) =>
    prev.obstacle.type === next.obstacle.type &&
    prev.obstacle.isSwooping === next.obstacle.isSwooping &&
    prev.obstacle.seagullType === next.obstacle.seagullType &&
    prev.obstacle.rotation === next.obstacle.rotation
);
